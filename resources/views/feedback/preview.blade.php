<x-feedback-layout>
    <x-slot name="projectName">{{ $project->name }}</x-slot>
    <x-slot name="projectDesc">{{ $project->desc }}</x-slot>
    @foreach(json_decode($feedbackForm->config) as $config)
    ...
    <div>
        <button class="bg-emerald-600 border border-emerald-700 py-3 w-full rounded text-white hover:bg-emerald-700 active:bg-emerald-800">Submit your review</button>
    </div>
</x-feedback-layout>
